/**
 *  Copied and substantially modified from
    Simple Javascript Image Viewer
    Copyright (C) 2010  Munawwar Firoz
    (see full notice at end of document)
*/
...
/*Utility functions*/
//get the upper left corner in the window
...
window.onresize = function(){
	var windowD = windowWidthHeight();
	var width = windowD[0];
	var height = windowD[1];
	// adjust for axis sizes
	width -= parseInt(yAxisWidth);
	height -= parseInt(xAxisHeight);
	// compute inset and adjust
	debug.println("width,height: " + width + "," + height);
	var heatmapObj = document.getElementById('heatmap');
	if (heatmapObj) {
		var leftTop = getObjectXY(heatmapObj);
		var w = leftTop[0];
		var h = leftTop[1];
		width -= 4*w;
		height -= 4*h;
	...
	height = height+"px";
	if (mainViewer) {
		mainViewer.resetFrame(width,height);
		mainViewer.fitToFrame();
	}
	if (xAxisViewer) {
		xAxisViewer.resetFrame(width,xAxisHeight);
		xAxisViewer.fitToFrame();
	}
	if (yAxisViewer) {
		yAxisViewer.resetFrame(yAxisWidth,height);
		yAxisViewer.fitToFrame();
	}
//	debug.println("Window width height: " + windowD[0] + "," + windowD[1]);
}
...
function debug_msgs() {
	...
	this.println=function(string) {
		if (this.counter > debug_limit) this.clear();
		this.print(string+'<br />');
		this.counter++;
	}
}
...
function start(width,height){
	...
	var xaxis=new viewer({ 
			name: 'xaxis', 
			image: xaxis_img, 
			frame: [mainWidth,xAxisHeight],
			track: 'width'
		});
	var yaxis_img=document.getElementById("yaxis");
		yaxis=new viewer({ 
			name: 'yaxis', 
			image: yaxis_img, 
			frame: [yAxisWidth,mainHeight],
			track: 'height'
		});
	main.setUpTrackers(xaxis,yaxis);
	xaxis.setUpTrackers(main,null);
	yaxis.setUpTrackers(main,null);
	main.frameElement.focus();
	
	// global handles (used by window resizing)
	mainViewer = main;
	xAxisViewer = xaxis;
	yAxisViewer = yaxis;
...
function viewer(arguments) //argument array
{
	...
	/*
		...
		Common to both:
			frame - An array of format [width, height]
				width and height are strings like '400px' or '100%'.			
			zoomFactor (optional) - number - Decides the amount zoomed per zoom event. Default is set to 10 (in %)
			maxZoom (optional) - Sets the makimum image e.g. ['500px,'500px']
	*/
	var key;
	...
	// debug.println("New Viewer: name = " + name);
	...
	self.setUpTrackers = function(viewer1,viewer2) {
		if (viewer1) tracker1 = viewer1;
		if (viewer2) tracker2 = viewer2;
//		if (tracker1) debug.println("Set up tracker: " 
//			+ tracker1.getName() + " tracks " + self.getName());
//		if (tracker2) debug.println("Set up tracker: " 
//			+ tracker2.getName() + " tracks " + self.getName());
	}
	...
	self.setDimension = function(width,height) { //width and height of image
		if (tracker1) tracker1.setDimensionUntracked(width,height);
		if (tracker2) tracker2.setDimensionUntracked(width,height);
		self.setDimensionUntracked(width,height);
	}
	self.setDimensionUntracked = function(width,height) {
		// set, without tracking, to avoid a->b, then b->a, recursion
		var f = this.getFrameDimension();
		if (track && (track == 'height')) width = f[0];
		if (track && (track == 'width')) height = f[1];
	...
	self.setPosition = function(x,y) { //x and y coordinate of image
		if (tracker1) tracker1.setPositionUntracked(x,y);
		if (tracker2) tracker2.setPositionUntracked(x,y);
		self.setPositionUntracked(x,y);
	}
	self.setPositionUntracked = function(x,y) {
		// set, without tracking, to avoid a->b, then b->a, recursion
		if (track && (track == 'height')) x=0;
		if (track && (track == 'width')) y=0;
	...
	self.setMouseCursor = function() {
		...
		if(dimension[0]>frameDimension[0] && dimension[1]>frameDimension[1])
			cursor='move';
		else if(dimension[0]>frameDimension[0] || self.getName()=='xaxis')
			cursor='e-resize';
		else if(dimension[1]>frameDimension[1] || self.getName()=='yaxis')
			cursor='n-resize';
	...
	self.maxZoomCheck = function(width,height) {
		...
		debug.println("Width,maxWidth: " + width + "," + maxZoom[0]);
		debug.println("Height,maxHeight: " + height + "," + maxZoom[1]);
	...
	self.resetFrame = function(newWidth, newHeight) {
		debug.println(self.getName() + " - old frame: " + frame[0] + "," + frame[1]);
		debug.println(self.getName() + " - new frame: " + newWidth + "," + newHeight);
		frame[0] = newWidth;
		frame[1] = newHeight;
	...
	self.zoomTo = function(newZoomLevel, x, y) {
		...
		*/
		...
		if(!self.maxZoomCheck(dimension[0],dimension[1])) {
			...
			dimension = self.limitToMaxZoom(dimension[0],dimension[1]);
	...
	self.limitToMaxZoom= function(width,height) {
		return [width,height];
	}
 ...
 *  Copied and substantially modified from
    Simple Javascript Image Viewer
    Copyright (C) 2010  Munawwar Firoz
	http://www.spictrading.com/viewer/home.php
